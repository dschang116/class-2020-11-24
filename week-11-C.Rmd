...
```{r}
new_obs <- tibble(treatment = c("Control", "Civic Duty", "Hawthorne", "Self","Neighbors"),
                  solo = TRUE,
                  primary_04 = "Yes")
...
posterior_epred(fit_1, newdata = new_obs) %>%
  ...
  mutate(diff_cd = `2` - `1`,
         ...
         diff_self = `4` - `1`,
         diff_nei = `5` - `1`) %>% 
  pivot_longer(cols = diff_cd:diff_nei,
               names_to = "treatment_type",
               values_to = "ate") %>% 
  mutate(treatment_type = fct_relevel(treatment_type, "diff_nei", after = 4)) %>% 
  ggplot(aes(x = ate, y = after_stat(count/sum(count)), fill = treatment_type)) +
    geom_histogram(bins = 100, alpha = 0.5, position = "identity") +
    theme_classic() +
    scale_fill_discrete(name = "Treatment Type",
                        labels = c("Civic Duty", "Hawthorne", "Neighbors", "Self")) +
